...
In this exercise, you will learn how to upgrade the cluster version from 1.31.1 to 1.31.5 using kubeadm. The cluster will consist of a single control plane node named `kube-control-plane`, and one worker node named `kube-worker-1`. The existing setup uses virtual machines (VMs) to emulate the cluster environment.
